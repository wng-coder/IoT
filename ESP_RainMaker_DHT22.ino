...
#define DHTPIN 22        // GPIO pin where DHT22 is connected (change if needed)
...
const long interval = 30000;  // Read sensor every 30 seconds

// Device pointers
static Device *my_device = NULL;
static Device *temp_sensor = NULL;
static Device *humidity_sensor = NULL;
...
void readDHT22() {
  ...
  if (temp_sensor) {
    temp_sensor->updateAndReportParam("Temperature", temperature);
  }
  if (humidity_sensor) {
    humidity_sensor->updateAndReportParam("Humidity", humidity);
  }
}
...
void setup() {
  ...
  temp_sensor = new Device("Temperature Sensor", "esp.device.temperature-sensor");
  if (temp_sensor) {
    temp_sensor->addNameParam();
    // Use explicit float values with 'f' suffix to avoid ambiguity
    Param temp_param("Temperature", "esp.param.temperature", value(0.0f), PROP_FLAG_READ);
    temp_param.addBounds(value(-40.0f), value(80.0f), value(0.1f));
    temp_param.addUIType(ESP_RMAKER_UI_TEXT);
    temp_sensor->addParam(temp_param);
    temp_sensor->assignPrimaryParam(temp_sensor->getParamByName("Temperature"));
  }

  // Create humidity sensor device
  humidity_sensor = new Device("Humidity Sensor", "esp.device.humidity-sensor");
  if (humidity_sensor) {
    humidity_sensor->addNameParam();
    // Use explicit float values with 'f' suffix to avoid ambiguity
    Param humidity_param("Humidity", "esp.param.humidity", value(0.0f), PROP_FLAG_READ);
    humidity_param.addBounds(value(0.0f), value(100.0f), value(0.1f));
    humidity_param.addUIType(ESP_RMAKER_UI_TEXT);
    humidity_sensor->addParam(humidity_param);
    humidity_sensor->assignPrimaryParam(humidity_sensor->getParamByName("Humidity"));
  }
  ...
  if (humidity_sensor) {
    my_node.addDevice(*humidity_sensor);
  }
